...
const yamlStringifyOptions: yaml.ToStringOptions = {
    blockQuote: false,
...
let data = {
    utils: {
        value: (value: any) => yaml
            .stringify(value, yamlStringifyOptions)
            .replace(/\r?\n/g, ' '),
        str: (value: any) => yaml
            .stringify('' + value, yamlStringifyOptions)
            .trim()
            .replace(/^"|"$/g, ''),
        include: (name: string) => fs.readFileSync(path.resolve(fileDir, name), 'utf-8'),
    },
...
function readWithTemplate(file: string) {
    let fileText = fs.readFileSync(file, 'utf-8');
    fileText = fileText
        .replace(/<%=([\s\S]+?)%>/g, '<%=utils.value($1)%>')
        .replace(/<%-([\s\S]+?)%>/g, '<%=$1%>')
        .replace(/<%!([\s\S]+?)%>/g, '<%=utils.str($1)%>')
    let compiled = template(fileText);
    let out = compiled(data);
    fs.writeFileSync(file + '.out.yml', out);
    return out;
}

let out = yaml.parse(readWithTemplate('template.yml'));
